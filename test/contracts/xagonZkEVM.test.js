...
describe('Xagon ZK-EVM', () => {
    ...
    it('should check initialize function', async () => {
        const XagonZkEVMFactory = await ethers.getContractFactory('XagonZkEVMMock');
        const xagonZkEVMContractInitialize = await upgrades.deployProxy(XagonZkEVMFactory, [], {
            initializer: false,
            constructorArgs: [
                xagonZkEVMGlobalExitRoot.address,
                maticTokenContract.address,
        ...
        await expect(xagonZkEVMContractInitialize.initialize(
            {
                admin: admin.address,
                trustedSequencer: trustedSequencer.address,
        ...
        await expect(
            xagonZkEVMContractInitialize.initialize(
                {
                    admin: admin.address,
                    trustedSequencer: trustedSequencer.address,
                    pendingStateTimeout: pendingStateTimeoutDefault,
        ...
        ).to.emit(xagonZkEVMContractInitialize, 'UpdateZkEVMVersion').withArgs(0, forkID, version);
    });
    ...
    it('should check setters of admin', async () => {
        ...
        // setTrustedSequencer
        await expect(xagonZkEVMContract.setTrustedSequencer(deployer.address))
            .to.be.revertedWith('OnlyAdmin');
        await expect(
            xagonZkEVMContract.connect(admin).setTrustedSequencer(deployer.address),
        ).to.emit(xagonZkEVMContract, 'SetTrustedSequencer').withArgs(deployer.address);
        expect(await xagonZkEVMContract.trustedSequencer()).to.be.equal(deployer.address);

        // setTrustedSequencerURL
        const url = 'https://test';
        await expect(xagonZkEVMContract.setTrustedSequencerURL(url))
            .to.be.revertedWith('OnlyAdmin');
        await expect(
            xagonZkEVMContract.connect(admin).setTrustedSequencerURL(url),
        ).to.emit(xagonZkEVMContract, 'SetTrustedSequencerURL').withArgs(url);
        expect(await xagonZkEVMContract.trustedSequencerURL()).to.be.equal(url);

        // setTrustedAggregator
        const newTrustedAggregator = deployer.address;
        await expect(xagonZkEVMContract.setTrustedAggregator(newTrustedAggregator))
            .to.be.revertedWith('OnlyAdmin');
        await expect(
            xagonZkEVMContract.connect(admin).setTrustedAggregator(newTrustedAggregator),
        ).to.emit(xagonZkEVMContract, 'SetTrustedAggregator').withArgs(newTrustedAggregator);
        expect(await xagonZkEVMContract.trustedAggregator()).to.be.equal(newTrustedAggregator);

        // setTrustedAggregatorTimeout
        await expect(xagonZkEVMContract.setTrustedAggregatorTimeout(trustedAggregatorTimeoutDefault))
            .to.be.revertedWith('OnlyAdmin');

        await expect(xagonZkEVMContract.connect(admin).setTrustedAggregatorTimeout(HALT_AGGREGATION_TIMEOUT + 1))
            .to.be.revertedWith('TrustedAggregatorTimeoutExceedHaltAggregationTimeout');

        await expect(xagonZkEVMContract.connect(admin).setTrustedAggregatorTimeout(trustedAggregatorTimeoutDefault))
        ...
        await expect(
            xagonZkEVMContract.connect(admin).setTrustedAggregatorTimeout(newTrustedAggregatorTimeout),
        ).to.emit(xagonZkEVMContract, 'SetTrustedAggregatorTimeout').withArgs(newTrustedAggregatorTimeout);
        expect(await xagonZkEVMContract.trustedAggregatorTimeout()).to.be.equal(newTrustedAggregatorTimeout);

        // setPendingStateTimeoutDefault
        await expect(xagonZkEVMContract.setPendingStateTimeout(pendingStateTimeoutDefault))
            .to.be.revertedWith('OnlyAdmin');

        await expect(xagonZkEVMContract.connect(admin).setPendingStateTimeout(HALT_AGGREGATION_TIMEOUT + 1))
            .to.be.revertedWith('PendingStateTimeoutExceedHaltAggregationTimeout');

        await expect(xagonZkEVMContract.connect(admin).setPendingStateTimeout(pendingStateTimeoutDefault))
            .to.be.revertedWith('NewPendingStateTimeoutMustBeLower');

        const newPendingStateTimeoutDefault = pendingStateTimeoutDefault - 1;
        await expect(
            xagonZkEVMContract.connect(admin).setPendingStateTimeout(newPendingStateTimeoutDefault),
        ).to.emit(xagonZkEVMContract, 'SetPendingStateTimeout').withArgs(newPendingStateTimeoutDefault);
        expect(await xagonZkEVMContract.pendingStateTimeout()).to.be.equal(newPendingStateTimeoutDefault);

        // setMultiplierBatchFee
        const newMultiplierBatchFee = 1023;
        await expect(xagonZkEVMContract.connect(admin).setMultiplierBatchFee(newMultiplierBatchFee + 1))
            .to.be.revertedWith('InvalidRangeMultiplierBatchFee');

        await expect(
            xagonZkEVMContract.connect(admin).setMultiplierBatchFee(newMultiplierBatchFee),
        ).to.emit(xagonZkEVMContract, 'SetMultiplierBatchFee').withArgs(newMultiplierBatchFee);
        expect(await xagonZkEVMContract.multiplierBatchFee()).to.be.equal(newMultiplierBatchFee);

        // setVerifyBatchTimeTarget
        const newVerifyBatchTimeTarget = 100;

        await expect(xagonZkEVMContract.connect(admin).setVerifyBatchTimeTarget(60 * 60 * 24 + 1)) // more than 1 day
            .to.be.revertedWith('InvalidRangeBatchTimeTarget');

        await expect(
            xagonZkEVMContract.connect(admin).setVerifyBatchTimeTarget(newVerifyBatchTimeTarget),
        ).to.emit(xagonZkEVMContract, 'SetVerifyBatchTimeTarget').withArgs(newVerifyBatchTimeTarget);
        expect(await xagonZkEVMContract.verifyBatchTimeTarget()).to.be.equal(newVerifyBatchTimeTarget);

        // setPendingStateTimeoutDefault
        const newForceBatchTimeout = 0;
        await expect(xagonZkEVMContract.setForceBatchTimeout(newForceBatchTimeout))
            .to.be.revertedWith('OnlyAdmin');

        await expect(xagonZkEVMContract.connect(admin).setForceBatchTimeout(HALT_AGGREGATION_TIMEOUT + 1))
            .to.be.revertedWith('InvalidRangeForceBatchTimeout');

        await expect(xagonZkEVMContract.connect(admin).setForceBatchTimeout(FORCE_BATCH_TIMEOUT))
            .to.be.revertedWith('InvalidRangeForceBatchTimeout');
        await expect(
            xagonZkEVMContract.connect(admin).setForceBatchTimeout(newForceBatchTimeout),
        ).to.emit(xagonZkEVMContract, 'SetForceBatchTimeout').withArgs(newForceBatchTimeout);
        expect(await xagonZkEVMContract.forceBatchTimeout()).to.be.equal(newForceBatchTimeout);

        // Activate force batches
        await expect(xagonZkEVMContract.activateForceBatches())
            .to.be.revertedWith('OnlyAdmin');

        // Check force batches are unactive
        await expect(xagonZkEVMContract.forceBatch('0x', 0))
            .to.be.revertedWith('ForceBatchNotAllowed');
        await expect(xagonZkEVMContract.sequenceForceBatches([]))
            .to.be.revertedWith('ForceBatchNotAllowed');

        await expect(
            xagonZkEVMContract.connect(admin).activateForceBatches(),
        ).to.emit(xagonZkEVMContract, 'ActivateForceBatches');
        await expect(xagonZkEVMContract.connect(admin).activateForceBatches())
            .to.be.revertedWith('ForceBatchesAlreadyActive');

        expect(await xagonZkEVMContract.isForcedBatchDisallowed()).to.be.equal(false);

        // Transfer admin role

        // First set pending Admin
        expect(await xagonZkEVMContract.pendingAdmin()).to.be.equal(ethers.constants.AddressZero);
        await expect(xagonZkEVMContract.transferAdminRole(deployer.address))
            .to.be.revertedWith('OnlyAdmin');

        await expect(
            xagonZkEVMContract.connect(admin).transferAdminRole(deployer.address),
        ).to.emit(xagonZkEVMContract, 'TransferAdminRole').withArgs(deployer.address);
        expect(await xagonZkEVMContract.pendingAdmin()).to.be.equal(deployer.address);

        // Accept transfer admin
        expect(await xagonZkEVMContract.admin()).to.be.equal(admin.address);
        await expect(xagonZkEVMContract.connect(admin).acceptAdminRole())
            .to.be.revertedWith('OnlyPendingAdmin');

        await expect(
            xagonZkEVMContract.connect(deployer).acceptAdminRole(),
        ).to.emit(xagonZkEVMContract, 'AcceptAdminRole').withArgs(deployer.address);
        expect(await xagonZkEVMContract.admin()).to.be.equal(deployer.address);
    });
    ...
    it('should check state roots inside prime', async () => {
        ...
        for (let i = 0; i < validRoots.length; i++) {
            expect(await xagonZkEVMContract.checkStateRootInsidePrime(validRoots[i])).to.be.equal(true);
        }

        for (let i = 0; i < aliasInvalidRoots.length; i++) {
            expect(await xagonZkEVMContract.checkStateRootInsidePrime(aliasInvalidRoots[i])).to.be.equal(false);
        }
    });
    ...
    it('should sequence a batch as trusted sequencer', async () => {
        ...
        await expect(xagonZkEVMContract.sequenceBatches([sequence], trustedSequencer.address))
        ...
        await expect(xagonZkEVMContract.connect(trustedSequencer).sequenceBatches([], trustedSequencer.address))
        ...
        // Sequence batch
        await expect(xagonZkEVMContract.connect(trustedSequencer).sequenceBatches([sequence], deployer.address))
        ...
        // Check batch mapping
        const sequencedBatchData = await xagonZkEVMContract.sequencedBatches(1);
        const batchAccInputHash = sequencedBatchData.accInputHash;

        const batchAccInputHashJs = calculateAccInputHash(
            (await xagonZkEVMContract.sequencedBatches(0)).accInputHash,
    ...
    it('sequenceBatches should sequence multiple batches', async () => {
        ...
        const maticAmount = (await xagonZkEVMContract.batchFee()).mul(2);
        ...
        // Sequence batches
        await expect(xagonZkEVMContract.connect(trustedSequencer).sequenceBatches([sequence, sequence2], trustedSequencer.address))
            .to.emit(xagonZkEVMContract, 'SequenceBatches')
        ...
        const sequencedBatchData2 = await xagonZkEVMContract.sequencedBatches(2);
    ...
    it('force batches through smart contract', async () => {
        ...
        const maticAmount = await xagonZkEVMContract.getForcedBatchFee();
        const lastGlobalExitRoot = await xagonZkEVMGlobalExitRoot.getLastGlobalExitRoot();
        ...
        // Approve matic
        const approveTx = await maticTokenContract.populateTransaction.approve(xagonZkEVMContract.address, maticAmount);
        ...
        // Force batch
        const lastForcedBatch = (await xagonZkEVMContract.lastForceBatch()) + 1;

        const forceBatchTx = await xagonZkEVMContract.populateTransaction.forceBatch(l2txDataForceBatch, maticAmount);
        await expect(sendDataContract.sendData(forceBatchTx.to, forceBatchTx.data))
            .to.emit(xagonZkEVMContract, 'ForceBatch')
    ...
    it('sequenceBatches should sequence multiple batches and force batches', async () => {
        ...
        await expect(
            maticTokenContract.approve(xagonZkEVMContract.address, maticAmount),
        ).to.emit(maticTokenContract, 'Approval');
        ...
        // Force batch
        await expect(xagonZkEVMContract.forceBatch(l2txDataForceBatch, maticAmount))
            .to.emit(xagonZkEVMContract, 'ForceBatch')
            .withArgs(lastForcedBatch, lastGlobalExitRoot, deployer.address, '0x');

        // sequence 2 batches
        const l2txData = '0x1234';
        const maticAmountSequence = (await xagonZkEVMContract.batchFee()).mul(1);
        ...
        // Approve tokens
        await expect(
            maticTokenContract.connect(trustedSequencer).approve(xagonZkEVMContract.address, maticAmountSequence),
        ).to.emit(maticTokenContract, 'Approval');
        ...
        // Check batch mapping
        const batchAccInputHash = (await xagonZkEVMContract.sequencedBatches(1)).accInputHash;
        // Only last batch is added to the mapping
        expect(batchAccInputHash).to.be.equal(ethers.constants.HashZero);
        ...
        const batchData2 = await xagonZkEVMContract.sequencedBatches(2);
    ...
    it('should force a batch of transactions', async () => {
        ...
        expect(maticAmount.toString()).to.be.equal((await xagonZkEVMContract.getForcedBatchFee()).toString());
        ...
        await expect(xagonZkEVMContract.forceBatch(l2txData, maticAmount.sub(1)))
        ...
        const lastForceBatch = await xagonZkEVMContract.lastForceBatch();
        ...
        const batchHash = await xagonZkEVMContract.forcedBatches(1);
    ...
    it('should sequence force batches using sequenceForceBatches', async () => {
        ...
        const forceBatchHash = await xagonZkEVMContract.forcedBatches(1);
        ...
        expect(await xagonZkEVMContract.lastForceBatchSequenced()).to.be.equal(0);
        expect(await xagonZkEVMContract.lastForceBatch()).to.be.equal(1);
        expect(await xagonZkEVMContract.lastBatchSequenced()).to.be.equal(0);
        ...
        await expect(xagonZkEVMContract.sequenceForceBatches(Array(2).fill(forceBatchStruct)))
            .to.be.revertedWith('ForceBatchesOverflow');

        // revert because the timeout is not expired
        await expect(xagonZkEVMContract.sequenceForceBatches([forceBatchStruct]))
        ...
        await expect(xagonZkEVMContract.sequenceForceBatches([forceBatchStructBad]))
            .to.be.revertedWith('ForcedDataDoesNotMatch');
        forceBatchStructBad.minForcedTimestamp -= 1;
        ...
        await expect(xagonZkEVMContract.sequenceForceBatches([forceBatchStruct]))
            .to.emit(xagonZkEVMContract, 'SequenceForceBatches')
            .withArgs(1);

        const timestampSequenceBatch = (await ethers.provider.getBlock()).timestamp;

        expect(await xagonZkEVMContract.lastForceBatchSequenced()).to.be.equal(1);
        expect(await xagonZkEVMContract.lastForceBatch()).to.be.equal(1);
        expect(await xagonZkEVMContract.lastBatchSequenced()).to.be.equal(1);
    ...
    it('should verify a sequenced batch using verifyBatchesTrustedAggregator', async () => {
        ...
        const numBatch = (await xagonZkEVMContract.lastVerifiedBatch()) + 1;
        ...
        await expect(
            xagonZkEVMContract.connect(deployer).verifyBatchesTrustedAggregator(
                pendingState,
                numBatch - 1,
                numBatch - 1,
                newLocalExitRoot,
        ...
        ).to.emit(xagonZkEVMContract, 'VerifyBatchesTrustedAggregator')
    ...
    it('should verify forced sequenced batch using verifyBatchesTrustedAggregator', async () => {
        ...
        ).to.emit(xagonZkEVMContract, 'VerifyBatch')
            .withArgs(numBatch, trustedAggregator.address)
            .to.emit(maticTokenContract, 'Transfer')
            .withArgs(xagonZkEVMContract.address, trustedAggregator.address, maticAmount);
    ...
    it('should match the computed SC input with the Js input', async () => {
        ...
        const sentBatchHash = (await xagonZkEVMContract.sequencedBatches(lastBatchSequenced + 1)).accInputHash;
        const oldAccInputHash = (await xagonZkEVMContract.sequencedBatches(0)).accInputHash;
        ...
        const currentStateRoot = await xagonZkEVMContract.batchNumToStateRoot(0);
        ...
        const circuitInpuSnarkSC = await xagonZkEVMContract.getNextSnarkInput(
            pendingStateNum,
            numBatch - 1,
            numBatch,
            newLocalExitRoot,
    ...
    it('should match the computed SC input with the Js input in force batches', async () => {
        ...
        const lastForcedBatch = (await xagonZkEVMContract.lastForceBatch()).toNumber() + 1;
        await expect(xagonZkEVMContract.forceBatch(l2txData, maticAmount))
            .to.emit(xagonZkEVMContract, 'ForceBatch')
            .withArgs(lastForcedBatch, lastGlobalExitRoot, deployer.address, '0x');
    ...
    it('should verify a sequenced batch using verifyBatches', async () => {
        ...
        const currentBatchFee = await xagonZkEVMContract.batchFee();
        ...
        expect(lastPendingstate).to.be.equal(await xagonZkEVMContract.lastPendingState());

        const pendingStateData = await xagonZkEVMContract.pendingStateTransitions(lastPendingstate);
        ...
        expect(0).to.be.equal(await xagonZkEVMContract.lastVerifiedBatch());

        // Pending state can't be 0
        await expect(
            xagonZkEVMContract.consolidatePendingState(0),
        ).to.be.revertedWith('PendingStateInvalid');

        // Pending state does not exist
        await expect(
            xagonZkEVMContract.consolidatePendingState(2),
        ).to.be.revertedWith('PendingStateInvalid');

        // Not ready to be consolidated
        await expect(
            xagonZkEVMContract.consolidatePendingState(lastPendingstate),
        ...
        await expect(
            xagonZkEVMContract.consolidatePendingState(1),
        ).to.be.revertedWith('PendingStateInvalid');

        // Fee es divided because is was fast verified
        const multiplierFee = await xagonZkEVMContract.multiplierBatchFee();
        expect((currentBatchFee.mul(1000)).div(multiplierFee)).to.be.equal(await xagonZkEVMContract.batchFee());

        // Check pending state variables
        expect(1).to.be.equal(await xagonZkEVMContract.lastVerifiedBatch());
        expect(newStateRoot).to.be.equal(await xagonZkEVMContract.batchNumToStateRoot(1));
        expect(1).to.be.equal(await xagonZkEVMContract.lastPendingStateConsolidated());
    });
    ...
    it('should test the pending state properly', async () => {
        ...
        let currentPendingStateData = await xagonZkEVMContract.pendingStateTransitions(currentPendingState);
        ...
        expect(newStateRoot).to.be.equal(await xagonZkEVMContract.batchNumToStateRoot(currentVerifiedBatch));
        ...
        expect(ethers.constants.HashZero).to.be.equal(await xagonZkEVMContract.batchNumToStateRoot(currentNumBatch));
        await expect(
            xagonZkEVMContract.connect(trustedAggregator).verifyBatchesTrustedAggregator(
                currentPendingState,
                currentNumBatch,
        ...
        currentPendingStateData = await xagonZkEVMContract.pendingStateTransitions(currentPendingState);
        expect(verifyTimestamp).to.be.equal(currentPendingStateData.timestamp);
        expect(newBatch).to.be.equal(currentPendingStateData.lastVerifiedBatch);
        expect(newLocalExitRoot).to.be.equal(currentPendingStateData.exitRoot);
        expect(newStateRoot).to.be.equal(currentPendingStateData.stateRoot);
        ...
        const firstPendingState = await xagonZkEVMContract.pendingStateTransitions(1);
        ...
        currentPendingState++;
        expect(currentPendingState).to.be.equal(await xagonZkEVMContract.lastPendingState());
        expect(currentPendingConsolidated).to.be.equal(await xagonZkEVMContract.lastPendingStateConsolidated());
        ...
        const secondPendingState = await xagonZkEVMContract.pendingStateTransitions(2);
        ...
        currentPendingConsolidated = await xagonZkEVMContract.lastPendingStateConsolidated();
        const lastPendingState = await xagonZkEVMContract.pendingStateTransitions(currentPendingState);
        ...
        const nextConsolidatedState = await xagonZkEVMContract.pendingStateTransitions(nextConsolidatedStateNum);
        ...
        await expect(
            xagonZkEVMContract.connect(admin).setPendingStateTimeout(0),
        ).to.emit(xagonZkEVMContract, 'SetPendingStateTimeout').withArgs(0);
    ...
    it('Activate emergency state due halt timeout', async () => {
        ...
        const sequencedTimestmap = Number((await xagonZkEVMContract.sequencedBatches(1)).sequencedTimestamp);
        const haltTimeout = HALT_AGGREGATION_TIMEOUT;

        // Try to activate the emergency state

        // Check batch is not sequenced
        await expect(xagonZkEVMContract.connect(aggregator1).activateEmergencyState(2))
            .to.be.revertedWith('BatchNotSequencedOrNotSequenceEnd');

        // Check batch is already verified
        await xagonZkEVMContract.setVerifiedBatch(1);
        await expect(xagonZkEVMContract.connect(aggregator1).activateEmergencyState(1))
            .to.be.revertedWith('BatchAlreadyVerified');
        await xagonZkEVMContract.setVerifiedBatch(0);
    ...
    it('Test batch fees properly', async () => {
        ...
        const verifyBatchTimeTarget = Number(await xagonZkEVMContract.verifyBatchTimeTarget());
        const currentTimestamp = (await ethers.provider.getBlock()).timestamp;

        const multiplierFee = ethers.BigNumber.from(await xagonZkEVMContract.multiplierBatchFee()); // 1002
        const bingNumber1000 = ethers.BigNumber.from(1000);

        // Create sequenced to update the fee
        await xagonZkEVMContract.setSequencedBatches(
            50,
            accInputData,
        ...
        // Assert currentFee
        let currentBatchFee = await xagonZkEVMContract.batchFee();
        ...
        await xagonZkEVMContract.updateBatchFee(100);

        // Fee does not change since there are the same batches above than below
        expect(await xagonZkEVMContract.batchFee()).to.be.equal(currentBatchFee);
        ...
        expect(currentBatchFee).to.be.equal(await xagonZkEVMContract.batchFee());

        // Check the fee is now below
        await xagonZkEVMContract.setSequencedBatches(50, accInputData, currentTimestamp + verifyBatchTimeTarget * 2, 0); // Below
