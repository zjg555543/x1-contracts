    ...
    IPolygonZkEVMBridgeL2
{
    ...
    function claimAsset(
        ...
        if (originTokenAddress == address(0)) {
            ...
            if (
                ...
                if (originNetwork == networkID) // Transfer tokens
                {
    ...
    function setL2TokenBridgePermission(
        address token,
        bool allowed
    ) external onlyPolygonZkEVM {
        require(token != address(0), "Invalid token address");
        require(
            allowed != isTokenAllowed[token],
            "Token already has the permission"
        );
    ...
    function setAllL2TokensAllowed(bool allowed) external onlyPolygonZkEVM {
        require(
            allowed != isAllL2TokensAllowed,
            "All L2 tokens already have the permission"
        );
