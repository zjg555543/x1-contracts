 ...
 */
contract XagonZkEVMTimelock is TimelockController {
    // Xagon ZK-EVM address. Will be used to check if it's on emergency state.
    XagonZkEVM public immutable xagonZkEVM;
     ...
     * @param _xagonZkEVM xagonZkEVM address
     **/
    constructor(
        uint256 minDelay,
     ...
     * If Xagon ZK-EVM is on emergency state the minDelay will be 0 instead.
     */
    function getMinDelay() public view override returns (uint256 duration) {
        if (address(xagonZkEVM) != address(0) && xagonZkEVM.isEmergencyState()) {
            return 0;
        } else {
